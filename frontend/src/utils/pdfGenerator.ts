...
export const generateEmployeeDocumentPDF = async (
  ...
  // Employee info section (selaras dengan detail di UI, layout rapi)
  const drawEmployeeInfo = () => {
    ...
    addLine('Nama Lengkap', data.employee.fullName);
    addLine('Email', data.employee.username);
    addLine('NIP', data.employee.nip);
    addLine('NIK', data.employee.nik);
    addLine('Jabatan', data.employee.position);
    addLine('Departemen', data.employee.department);
    addLine('Jenis Kelamin', data.employee.gender);
    addLine('Tempat Lahir', data.employee.placeOfBirth);

    const dob = data.employee.dateOfBirth
      ? new Date(data.employee.dateOfBirth).toLocaleDateString('id-ID')
      : undefined;
    const join = data.employee.joinDate
      ? new Date(data.employee.joinDate).toLocaleDateString('id-ID')
      : undefined;
    addLine('Tanggal Lahir', dob);
    addLine('Tanggal Bergabung', join);
    addLine('Status Perkawinan', data.employee.maritalStatus);
    addLine('Agama', data.employee.religion);
    addLine('Pendidikan Terakhir', data.employee.lastEducation);
    addLine('Pangkat / Golongan', data.employee.rankGolongan);
    addLine('Status Kepegawaian', data.employee.employmentStatus);
...
export const generateAllEmployeesDocumentPDF = async (
  ...
  // Summary table untuk setiap pegawai (dilengkapi biodata ringkas)
  data.employees.forEach((empData, empIndex) => {
    ...
    addLine('Email', e.username);
    addLine('NIP', e.nip);
    addLine('Jabatan', e.position);
    addLine('Departemen', e.department);
    if (typeof e.remainingLeaveDays === 'number') {
      addLine('Sisa Cuti (hari)', e.remainingLeaveDays);
    }
    yPos += 2;
