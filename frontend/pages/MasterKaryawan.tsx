...
import { getUsers, saveUser, deleteUser, getDocTypes, getSettings } from '../store';
...
const MasterKaryawan: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      const [usersData, docTypesData, settingsData] = await Promise.all([
        getUsers(),
        getDocTypes(),
        getSettings()
      ]);
              ...
              <tr key={user.id} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td className="px-8 py-5">
                  <div className="flex items-center gap-2">
                    <button onClick={() => openModal(user)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit Data & Password">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
            <div className="p-8 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <h2 className="text-xl font-black text-slate-800">{editingUser ? 'Edit Kredensial & Profil' : 'Daftarkan Pegawai Baru'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="pt-4 flex gap-4">
                <button type="button" onClick={closeModal} className="flex-1 py-4 bg-slate-100 text-slate-600 font-bold rounded-2xl hover:bg-slate-200 transition-colors">
                  Batal
                </button>
                <button type="submit" className="flex-1 py-4 bg-teal-500 text-white font-bold rounded-2xl shadow-lg shadow-teal-100 hover:bg-teal-600 transition-all">
